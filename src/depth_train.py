...
from depth_model import build_model
import utils
import train_utils
...
if __name__ == "__main__":
    ...
    depth_dataset = utils.create_depth_dataset(train_data_path)
    test_dataset = utils.create_depth_dataset(test_data_path)

    print(f'done! took {time.time()-start_time:.2f} seconds')

    print('----------------')
    print('CLASS NAMES: ')
    print(depth_dataset.class_names)
    print('----------------')

    num_train_examples = len(depth_dataset)
    ...
    train_dataset = depth_dataset.cache().shuffle(BUFFER_SIZE).batch(BATCH_SIZE)
    test_dataset = test_dataset.batch(BATCH_SIZE)

    # build model
    IM_SIZE = (200, 200)
    NUM_CLASSES = len(depth_dataset.class_names)
    ...
    # with strategy.scope():
    depth_model = build_model(IM_SIZE, NUM_CLASSES)
    try:
    ...
    print(f'done! took {time.time()-start_time:.2f} seconds')
    depth_model.summary()  # print model summary

    # log trainable parameters of model
    trainable_count = np.sum([K.count_params(w) for w in depth_model.trainable_weights])
    ...
    if weights_loaded:
        ...
        loss, acc = depth_model.evaluate(test_dataset)
    ...
    for epoch in range(1, epochs + 1):
        # training batches
        losses_train = train_utils.train_epoch(depth_model, train_dataset, epoch, optimizer, loss, train_metric, max_steps=None, logger=logger)
        ...
        losses_val = train_utils.validate_epoch(depth_model, test_dataset, epoch, loss, test_metric, max_steps=None, logger=logger)
        ...
        print('saving model...')
        depth_model.save_weights(f'outputs/checkpoints/epoch{epoch}_weights.h5')
    ...
    depth_model.save('outputs/depth_model')
